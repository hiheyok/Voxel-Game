...
void Chunk::GenerateDebug() {
	if (Position.y == 0) {
		...
		for (const auto& b : Blocks.BlockTypeData) {
			SetBlockUnsafe(i, (i & 0b1111), 1, i >> 4);
			i++;
		}
	}
	...
	for (int x = 0; x < 16; x++) {
		for (int y = 0; y < 16; y++) {
			for (int z = 0; z < 16; z++) {
				...
				if ((GetBlock(x, y, z) == Blocks.RED_STAINED_GLASS) || (GetBlock(x, y, z) == Blocks.BLACK_STAINED_GLASS)) {
					continue;
				}
					...
					if (((int)globalPos.z % (Unit * (int)zoom)) == 0) {
						for (int i = -UnitSize; i <= UnitSize; i++) {
							for (int j = -UnitSize; j <= UnitSize; j++) {
								SetBlock(Blocks.RED_STAINED_GLASS, x + j, y + i, z);
							}
						}
					}
					else {
						SetBlock(Blocks.BLACK_STAINED_GLASS, x, y, z);
					}
				...
				if (gz == 0 && gy == 0) {
					if (((int)globalPos.x % (Unit * (int)zoom)) == 0) {
						for (int i = -UnitSize; i <= UnitSize; i++) {
							for (int j = -UnitSize; j <= UnitSize; j++) {
								SetBlock(Blocks.RED_STAINED_GLASS, x, y + i, z + j);
							}
						}
					}
					else {
				...
				if (gz == 0 && gx == 0) {
					if (((int)globalPos.y % (Unit * (int)zoom)) == 0) {
						for (int i = -UnitSize; i <= UnitSize; i++) {
							for (int j = -UnitSize; j <= UnitSize; j++) {
								SetBlock(Blocks.RED_STAINED_GLASS, x + i, y, z + j);
							}
						}
					}
					else {
