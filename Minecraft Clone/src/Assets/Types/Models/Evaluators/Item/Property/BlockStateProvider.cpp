...
BlockStateProvider::BlockStateProvider(GameContext& context, const json& data)
    : PropertyValueProvider(context) {
  Parse(data);
}
BlockStateProvider::~BlockStateProvider() = default;

const Evaluator* BlockStateProvider::GetEvaluator(
    const EvaluatorContext&) const {
  return nullptr;
}

void BlockStateProvider::Parse(const json& data) {
  if (!data.contains("block_state_property") || !data.contains("cases")) {
    LOG_WARN("Incomplete block state provider.");
