...
ContextDimensionProvider::ContextDimensionProvider(GameContext& context,
                                                   const json& data)
    : PropertyValueProvider{context} {
  Parse(data);
}
ContextDimensionProvider::~ContextDimensionProvider() = default;

const Evaluator* ContextDimensionProvider::GetEvaluator(
    const EvaluatorContext&) const {
  return nullptr;
}

void ContextDimensionProvider::Parse(const json& data) {
  if (!data.contains("cases")) {
    LOG_WARN("Incomplete dimension provider.");
