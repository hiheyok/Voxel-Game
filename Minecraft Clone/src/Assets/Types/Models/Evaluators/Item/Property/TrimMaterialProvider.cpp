...
TrimMaterialProvider::TrimMaterialProvider(GameContext& context,
                                           const json& data)
    : PropertyValueProvider{context} {
  Parse(data);
}
TrimMaterialProvider::~TrimMaterialProvider() = default;

const Evaluator* TrimMaterialProvider::GetEvaluator(
    const EvaluatorContext&) const {
  return nullptr;
}

void TrimMaterialProvider::Parse(const json& data) {
  if (!data.contains("cases")) {
    LOG_WARN("Incomplete trim material provider.");
