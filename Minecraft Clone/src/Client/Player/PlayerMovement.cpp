...
void PlayerMovement::MovePlayer(Player* player, const UserInputs& inputs,
    ...
    if (enable_collusion_) {
        ...
        if ((time.y != -1.) && (time.y <= inputs.delta_)) {
            ...
            player->properties_.velocity_.x = player->properties_.velocity_.x *
                                              powf(1.f / 250.f, inputs.delta_);
            player->properties_.velocity_.z = player->properties_.velocity_.z *
                                              powf(1.f / 250.f, inputs.delta_);
        }
