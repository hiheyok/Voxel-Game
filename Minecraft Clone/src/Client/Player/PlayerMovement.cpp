...
float PlayerMovement::VelocityMovementCurve(float current, float max,
                                            float delta) {
  int current_time = static_cast<int>(-log(max - current) +
                                      log(max));  // TODO(hiheyok): Fix this

  int x = static_cast<int>(delta - log(max) - current_time);
...
void PlayerMovement::RotatePlayer(Player& player, const InputManager& inputs) {
  ...
  player.properties_.rotation_ +=
      inputs.GetMouseDisplacement() * vec2(1.0f, -1.0f) * kCamSens;
...
void PlayerMovement::MovePlayer(Player& player, const InputManager& inputs,
                                ClientCache& client_world) {
  vec3 front(cos(player.properties_.rotation_.x * kDegToRad) *
  ...
  if (inputs.CheckAction(InputAction::kJump) &&
      (client_world.collision_manager_.IsEntityOnGround(player) &&
       enable_collision_)) {
  ...
  if (enable_collision_) {
    ...
    vec3 time = client_world.collision_manager_.ComputeCollisionTimes(player);

    if ((time.x != -1.0f) && (time.x <= inputs.delta_)) {
      player.properties_.velocity_.x = 0;
    }
    if ((time.y != -1.0f) && (time.y <= inputs.delta_)) {
      player.properties_.velocity_.y = 0;

      player.properties_.velocity_.x =
          player.properties_.velocity_.x * powf(1.0f / 250.f, inputs.delta_);
      player.properties_.velocity_.z =
          player.properties_.velocity_.z * powf(1.0f / 250.f, inputs.delta_);
    }
    if ((time.z != -1.0f) && (time.z <= inputs.delta_)) {
      player.properties_.velocity_.z = 0;
    }
  }

  player.properties_.position_ +=
      player.properties_.velocity_ * inputs.delta_ / 2.0f;
  player.properties_.velocity_ =
      player.properties_.velocity_ * powf(3.0f / 25.0f, inputs.delta_);
}
