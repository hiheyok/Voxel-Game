...
BlockID WorldInteraction::GetBlock(Ray ray, ServerInterface* interface,
    ...
    if (cache->collusion_manager_.CheckRayIntersection(ray)) {
        BlockPos pos{(int)floor(ray.end_point_.x), (int)floor(ray.end_point_.y),
...
void WorldInteraction::BreakBlock(Ray ray, ServerInterface* interface,
    ...
    if (cache->collusion_manager_.CheckRayIntersection(ray)) {
        ...
        packet.pos_ =
            BlockPos{(int)floor(ray.end_point_.x), (int)floor(ray.end_point_.y),
                     (int)floor(ray.end_point_.z)};
...
void WorldInteraction::PlaceBlock(Ray ray, BlockID block,
    ...
    if (cache->collusion_manager_.CheckRayIntersection(ray)) {
        ...
        placePos[(int)floor(BounceSurface >> 1)] +=
            (BounceSurface & 0b1) -
            ((BounceSurface + 1) &
