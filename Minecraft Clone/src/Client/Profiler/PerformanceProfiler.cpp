...
void PerformanceProfiler::LoadCache() {
    ...
    for (const auto& [hash, time] : time_pass_cache_) {
        std::vector<std::string> tokens =
            string_tokenized_hash_container_[hash];
...
void PerformanceProfiler::Print() { root_.print(); }
...
void PerformanceProfiler::CombineCache(PerformanceProfiler profiler) {
    ...
    time_pass_cache_.insert(time_pass_cache_.end(),
                            profiler.time_pass_cache_.begin(),
                            profiler.time_pass_cache_.end());
    string_hash_container_.insert(profiler.string_hash_container_.begin(),
                                  profiler.string_hash_container_.end());
    string_tokenized_hash_container_.insert(
        profiler.string_tokenized_hash_container_.begin(),
        profiler.string_tokenized_hash_container_.end());
    CondenseCache();
}
