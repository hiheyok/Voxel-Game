...
std::unique_ptr<Mesh::ChunkVertexData> WorldRender::Worker(
    const ChunkPos& pos) {
    ...
    data->transparent_vertices_.resize(chunkMesher.transparent_face_count_ *
                                       12);
    data->position_ = pos;
...
void WorldRender::Start(GLFWwindow* window, ClientCache* cache,
    ...
    mesh_thread_pool_ = std::make_unique<
        ...
        threadCount, "Mesher",
        [this](const ChunkPos& input)
            -> std::unique_ptr<Mesh::ChunkVertexData> { return Worker(input); },
        250);
    ...
    renderer_->SetSettings(horizontal_render_distance_,
                           vertical_render_distance_, 90);
