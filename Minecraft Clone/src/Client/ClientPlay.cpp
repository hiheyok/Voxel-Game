...
ClientPlay::ClientPlay(ServerInterface* interface, Window* window,
    ...
    terrain_render_->Start(window->GetWindow(), &client_level_->cache,
                           profiler);
    framebuffer_->GenBuffer(window->GetProperties().window_size_x_,
                            window->GetProperties().window_size_y_,
                            (float)g_app_options.graphics_scale_, GL_RGB);
}
...
void ClientPlay::UpdateDebugStats() {
    ...
    if (client_level_->cache.CheckChunk(block_pos)) {
        light_lvl =
            client_level_->cache.GetChunk(block_pos)->lighting_->GetLighting(
                rel_block_pos);
    }
    ...
    debug_screen_->EditText(
        ...
        "VRAM Usage: " +
            std::to_string(
                (double)terrain_render_->renderer_->GetVRAMUsageFull() /
                1000000.0) +
            " MB");
    ...
    debug_screen_->EditText(
        ...
        "VRAM Fragmentation Rate: " +
            std::to_string(terrain_render_->renderer_->GetFragmentationRate() *
                           100) +
            "%");
    debug_screen_->EditText("Stat5",
                            "FPS: " + std::to_string(1.0 / frametime_));
    debug_screen_->EditText(
        "Stat6",
    ...
    debug_screen_->EditText(
        "Stat7", "Mesh Engine Queued: " +
                     std::to_string(terrain_render_->GetQueuedSize()));
    debug_screen_->EditText("Stat8",
                            "Light Level: " + std::to_string(light_lvl));
    debug_screen_->EditText(
        "Stat9", "Server Tick (MSPT): " + std::to_string(stats.mspt_));
    debug_screen_->EditText(
        "Stat10", "Chunk Count: " + std::to_string(stats.chunk_count_));
        ...
        switch (packet.type_) {
            ...
            case EntityUpdatePacket::ENTITY_INVENTORY_UPDATE: {
                const EntityUpdatePacket::EntityInventoryUpdate& data =
                    std::get<EntityUpdatePacket::EntityInventoryUpdate>(
                        packet.packet_);
                // N/A
                break;
            }
