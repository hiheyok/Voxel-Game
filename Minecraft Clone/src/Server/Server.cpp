...
void Server::Loop() {
    ...
    while (!stop_) {
        ...
        double time_left =
            (1.0 / (double)settings_->tick_rate_) - mspt_ / 1000.0;
    ...
    for (const auto& packet : playerPackets) {
        switch (packet.type_) {
            case PlayerPacket::GET_ITEM:
                ...
                {
                    const PlayerPacket::PlayerGetItem& getItem =
                        std::get<PlayerPacket::PlayerGetItem>(packet.packet_);
                    Entity* e = level_->main_world_->world_->GetEntity(
                        receiver->GetPlayerUUID());
                    Player* player = static_cast<Player*>(e);
                    player->entity_inventory_.SetSlot(getItem.slot_,
                                                      getItem.item_);
...
void Server::SendEntityUpdatePacket(ClientInterface* receiver) {
    ...
    std::vector<EntityProperty> entityUpdated =
        level_->main_world_->world_updater_->GetUpdatedEntities();
    std::vector<EntityUUID> entityDespawned =
...
void Server::SendChunkUpdatePacket(ClientInterface* receiver) {
    std::vector<ChunkPos> updatedChunks =
        level_->main_world_->world_updater_->GetUpdatedChunkPos();
    std::vector<ChunkPos> updatedLights =
