...
static constexpr std::array<string_view, 4> kSeverityStrings = {
...
void LogUtils::MainLogger(std::stop_token stoken) {
  ...
  while (!stoken.stop_requested()) {
    if (!logs_cache_.empty()) {
      ...
      string_view severity = kSeverityStrings[static_cast<size_t>(log.type_)];
      ...
      logs_cache_ = move(logs_);
      logs_.clear();
    }
  }
}

void LogUtils::Log(LogType type, string_view func, string&& msg) {
  LogData log;
  log.type_ = type;
  log.message_ = move(msg);
  log.time_ = std::chrono::system_clock::now();
  log.func_ = func;
  log.r_time_ = (high_resolution_clock::now() - start_time_).count();
  lock_guard<mutex> lock{mutex_};
  logs_.emplace_back(move(log));
  cv_.notify_one();
}
