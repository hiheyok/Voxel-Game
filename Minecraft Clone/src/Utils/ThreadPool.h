    ...
    while (!stop_.load()) {
        ...
        for (size_t i = 0; !stop_ && i < worker_count_; ++i) {
            ...
            worker_task_[i].insert(
                worker_task_[i].end(), internalTaskList.begin() + currIndex,
...
inline void ThreadPool<TaskIn, TaskOut>::Worker(int workerId) {
    ...
    while (!stop_.load()) {
        ...
        size_t i = batchIndex;
        for (; !stop_ &&
               i < std::min(workerTasks.size(), batchIndex + batch_size_);
             ++i) {
            if (stop_) break;
...

        {
            ...
            worker_output_[workerId].insert(
                worker_output_[workerId].end(),
