...
float CollusionDetector::TraceSingleAxisCollision(glm::vec3 origin,
    ...
    for (int i = 0; i < distance_test; i++) {
        ...
        BlockID b = cache_->GetBlock(
            BlockPos{floored_pos.x, floored_pos.y, floored_pos.z});
...
glm::dvec3 CollusionDetector::ComputeCollisionTimes(Entity* entity) {
    AABB hitbox =
        g_entity_list.GetEntity(entity->properties_.type_)->GetHitbox();

    glm::vec3 hitbox_start =
        entity->properties_.position_ - (hitbox.size_ / 2.f);
        ...
        for (int u = 0; u <= i_bound[u_axis]; u++) {
            for (int v = 0; v <= i_bound[v_axis]; v++) {
                ...
                float distance = TraceSingleAxisCollision(
                    origin_, direction, (int)floor(least_distance) + 2);

                if ((distance < least_distance) && (distance != -1.f))
                    least_distance = distance;

                if (distance ==
                    -1.f)  // -1.f means that it cannot find any blocks that
                ...
                float time =
                    abs(distance /
                        entity->properties_
                            .velocity_[axis]);  // This gets the time it takes
                                                // for the entity to travel that
                                                // distance
                ...
                if (least_time[axis] ==
                    -1.f)  // leasttime[axis] == -1.f means that a "time" value
                           // haven't been inputted yet
                    least_time[axis] = time;
            }
        }
    }
...
bool CollusionDetector::CheckRayIntersection(Ray& ray) {
    ...
    glm::vec3 delta_dist(abs(1 / delta[0]), abs(1 / delta[1]),
                         abs(1 / delta[2]));

    // Stepping Variable
    glm::vec3 side_dist(
        ((float)direction[0] * ((float)blockPos[0] - end_point[0]) +
         ((float)direction[0] * 0.5f) + 0.5f) *
            delta_dist[0],
        ((float)direction[1] * ((float)blockPos[1] - end_point[1]) +
         ((float)direction[1] * 0.5f) + 0.5f) *
            delta_dist[1],
        ((float)direction[2] * ((float)blockPos[2] - end_point[2]) +
         ((float)direction[2] * 0.5f) + 0.5f) *
            delta_dist[2]);
    ...
    while (iterations < kMaxIterations) {
        ...
        BlockID b =
            cache_->GetBlock(BlockPos{blockPos.x, blockPos.y, blockPos.z});
            ...
            for (int axis = 0; axis < 3; axis++) {
                if (mask[axis]) {
                    ray.bounce_surface_ =
                        delta[axis] < 0
                            ? axis * 2 + 1
                            : axis * 2;  // Set the surface it bounces off. + 1
                                         // means that the surface is behind,
                                         // bottom, etc
                    return true;
                }
            }
