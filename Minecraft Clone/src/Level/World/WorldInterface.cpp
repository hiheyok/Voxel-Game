...
BlockID WorldInterface::GetBlock(const BlockPos& pos) const {
    return chunks_->GetBlock(pos);
}

Chunk* WorldInterface::GetChunk(const ChunkPos& pos) const {
...
bool WorldInterface::CheckChunk(const ChunkPos& pos) const {
