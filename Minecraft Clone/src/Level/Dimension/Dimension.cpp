...
Dimension::Dimension(DimensionProperties properties, WorldGeneratorID generatorType) :
    ...
    world_updater_ = std::make_unique<WorldUpdater>(main_world_.get(), world_settings_);
    collusion_detector_ = std::make_unique<CollusionDetector>(main_world_->GetChunkMap());
...
WorldGeneratorID Dimension::GetGeneratorType() const {
    return generator_type_;
}

bool Dimension::CheckTickUsed(EventID id, const BlockPos& pos) const {//temp sol
...
void Dimension::Update() {
    ...
    std::vector<ChunkPos> requestedLights = world_updater_->GetRequestedLightUpdates();
    ...
    std::vector<std::unique_ptr<LightStorage>> lights = light_engine_->GetOutput();
