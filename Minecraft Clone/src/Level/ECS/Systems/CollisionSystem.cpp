...
CollisionSystem::CollisionSystem(GameContext& context, WorldInterface& world, EntitySystems& entity_systems)
...
void CollisionSystem::Tick() {
  ...
  for (auto& [uuid, collision_comp] : components_updated_) {
    ...
    collision_comp.standing_block = context_.blocks_->DIRT;
    continue;
    ...
    BlockPos block_below(static_cast<int>(std::floor(pos.x)),
                        block_below_y,
                        static_cast<int>(std::floor(pos.z)));
    
    BlockID block = world_.GetBlock(block_below);
    bool is_solid = context_.blocks_->GetBlockType(block)->properties_->is_solid_;
    ...
    bool is_grounded = is_solid && (distance_above_block >= 0.0f && distance_above_block <= 0.15f);

    // If entity just became grounded, snap it to the block surface and stop downward velocity
    if (is_grounded && !collision_comp.grounded) {
      transform_system.SetPosition(uuid, glm::vec3(pos.x, top_of_block_below, pos.z));
...
bool CollisionSystem::GetCollusionStatus(EntityUUID uuid, int side) const noexcept {
