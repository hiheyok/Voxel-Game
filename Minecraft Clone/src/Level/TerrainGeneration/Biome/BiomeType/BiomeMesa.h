...
class BiomeMesa : public Biome {
   ...
   public:
    ...
    void GenerateBands(long p_150619_1_) {
        ...
        clay_bands_offset_noise_ =
            std::make_unique<NoiseGeneratorPerlin>(random, 1);
        ...
        for (int l4 = 0; l4 < k3; ++l4) {
            ...
            for (int k1 = 0; j4 + k1 < 64 && k1 < 1; ++k1) {
                ...
                if (j4 + k1 > 1 && random.NextBoolean()) {
                    clay_bands_[j4 + k1 - 1] =
                        g_blocks.GRAY_TERRACOTTA;  // silver terracotta
                }
    ...
    BlockID GetBand(int p_180629_1_, int p_180629_2_, int p_180629_3_) {
        (void)p_180629_3_;
        int i = (int)std::round(
            clay_bands_offset_noise_->getValue((double)p_180629_1_ / 512.0,
                                               (double)p_180629_1_ / 512.0) *
            2.0);
        return clay_bands_[(p_180629_2_ + i + 64) % 64];
    }

    void GenTerrainBlocks(JavaRandom& rand, TallChunk* chunk, int x, int z,
                          double noiseVal, ChunkGeneratorSettings* settings_)
        override {  // doiuble check this again later
        if (clay_bands_.size() == 0 || world_seed_ != -1587754402) {
            GenerateBands(-1587754402);  // world seed here
        }

        if (pillar_noise_ == nullptr || pillar_roof_noise_ == nullptr ||
        ...
        if (bryce_pillars_) {
            ...
            double d0 = std::min(
                abs(noiseVal),
                pillar_noise_->getValue((double)i * 0.25, (double)j * 0.25));

            if (d0 > 0.0) {
                // double d1 = 0.001953125;
                double d2 = std::abs(pillar_roof_noise_->getValue(
                    (double)i * 0.001953125, (double)j * 0.001953125));
                d4 = d0 * d0 * 2.5;
        ...
        for (int j1 = 255; j1 >= 0; --j1) {
            if (chunk->GetBlockUnsafe(l1, j1, k1) == g_blocks.AIR &&
                j1 < (int)d4) {
                    ...
                    if (l == -1) {
                        ...
                        if (j1 >= i2 - 1) {
                            if (has_forest_ && j1 > 86 + k * 2) {
                                if (flag) {
                                    chunk->SetBlockUnsafe(l1, j1, k1,
                                                          g_blocks.COARSE_DIRT);
                                } else {
                                    chunk->SetBlockUnsafe(l1, j1, k1,
                                                          g_blocks.GRASS);
                                }
                            ...
                            if (iblockstate3 == g_blocks.WHITE_TERRACOTTA) {
                                chunk->SetBlockUnsafe(
                                    l1, j1, k1, g_blocks.ORANGE_TERRACOTTA);
                            }
                        }
                    } else if (l > 0) {
                        --l;
