 ...
 public:
  ...
  BlockID GetBand(int p_180629_1_, int p_180629_2_, int p_180629_3_) {
    ...
    int i = static_cast<int>(
        std::round(clay_bands_offset_noise_->getValue(
                       static_cast<double>(p_180629_1_) / 512.0,
                       static_cast<double>(p_180629_1_) / 512.0) *
                   2.0));
  ...
  void GenTerrainBlocks(JavaRandom& rand, TallChunk* chunk, int x, int z,
    ...
    if (bryce_pillars_) {
      int i = (x & -16) + (z & 15);
      int j = (z & -16) + (x & 15);
      double d0 = std::min(abs(noiseVal), pillar_noise_->getValue(
      ...
      if (d0 > 0.0) {
        ...
        double d2 = std::abs(
            pillar_roof_noise_->getValue(static_cast<double>(i) * 0.001953125,
                                         static_cast<double>(j) * 0.001953125));
        d4 = d0 * d0 * 2.5;
