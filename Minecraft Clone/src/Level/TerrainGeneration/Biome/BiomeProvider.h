...
class BiomeProvider {
 private:
  ...
  static int64_t biomeProviderSeed;

 public:
  BiomeProvider() {}

  BiomeProvider(int64_t seed, ChunkGeneratorSettings* settingsIn) {
    (void)seed;
    (void)settingsIn;
    g_logger.LogError("BiomeProvider::BiomeProvider", "Don't use this");
  }

  static void init(int64_t seed, ChunkGeneratorSettings* settingsIn) {
    std::lock_guard<std::mutex> lock{g_biome_lock};
    std::vector<GenLayer*> agenlayer =
        CombinedGenLayers::initializeAllBiomeGenerators(seed, settingsIn);
  ...
  static std::vector<Biome*> getBiomesForGeneration(int x, int z, int width,
                                                    int length) {
    size_t threadHash = std::hash<std::thread::id>{}(
        std::this_thread::get_id());  // Used to detect if a new thread is
                                      // running
    ...
    std::vector<int> aint =
        BiomeProvider::genBiomes[threadIndex]->getInts(x, z, width, length);
  ...
  static std::vector<Biome*> getBiomes(int x, int z, int width, int length) {
    ...
    std::vector<int> aint =
        biomeIndexLayer[threadIndex]->getInts(x, z, width, length);
...
inline std::vector<GenLayer*> BiomeProvider::genBiomes =
    std::vector<GenLayer*>();
inline std::vector<GenLayer*> BiomeProvider::biomeIndexLayer =
    std::vector<GenLayer*>();
inline ChunkGeneratorSettings* BiomeProvider::settings_ = nullptr;
inline int64_t BiomeProvider::biomeProviderSeed = 0;
inline FastHashMap<size_t, size_t> BiomeProvider::ThreadGenIndex =
    FastHashMap<size_t, size_t>();
