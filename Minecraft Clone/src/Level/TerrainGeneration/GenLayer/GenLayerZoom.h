...
class GenLayerZoom : public GenLayer {
   public:
    GenLayerZoom(long long baseSeedIn, GenLayer* parentIn)
        : GenLayer(baseSeedIn) {
        parent = parentIn;
    }
    ...
    std::vector<int> getInts(int areaX, int areaY, int areaWidth,
        ...
        for (int k1 = 0; k1 < l - 1; ++k1) {
            ...
            for (int k2 = aint[i2 + 0 + (k1 + 1) * k]; i2 < k - 1; ++i2) {
                initChunkSeed(static_cast<long long>(i2 + i) << 1,
                              static_cast<long long>(k1 + j) << 1);
        ...
        for (int j3 = 0; j3 < areaHeight; ++j3) {
            std::copy(aint1.begin() + (j3 + (areaY & 1)) * i1 + (areaX & 1),
    ...
    static GenLayer* magnify(long long p_75915_0_, GenLayer* p_75915_2_,
                             int p_75915_3_) {
