    ...
    for (block_pos.x = 0; block_pos.x < kChunkDim; block_pos.x++) {
        for (block_pos.y = 0; block_pos.y < kChunkDim; block_pos.y++) {
            for (block_pos.z = 0; block_pos.z < kChunkDim; block_pos.z++) {
                ...
                if ((chunk->GetBlock(block_pos) == g_blocks.RED_STAINED_GLASS) || (chunk->GetBlock(block_pos) == g_blocks.BLACK_STAINED_GLASS))
                    continue;
                    ...
                    if (((int)globalPos.z % (Unit * (int)zoom)) == 0) {
                        for (int i = -UnitSize; i <= UnitSize; i++) {
                            for (int j = -UnitSize; j <= UnitSize; j++) {
                                chunk->SetBlock(g_blocks.RED_STAINED_GLASS, block_pos + BlockPos{j, i, 0});
                            }
                        }
                    }
                    else {
                ...
                if (gz == 0 && gy == 0) {
                    if (((int)globalPos.x % (Unit * (int)zoom)) == 0) {
                        for (int i = -UnitSize; i <= UnitSize; i++) {
                            for (int j = -UnitSize; j <= UnitSize; j++) {
                                chunk->SetBlock(g_blocks.RED_STAINED_GLASS, block_pos + BlockPos{0, j, i});
                            }
                        }
                    }
                    else {
                ...
                if (gz == 0 && gx == 0) {
                    if (((int)globalPos.y % (Unit * (int)zoom)) == 0) {
                        for (int i = -UnitSize; i <= UnitSize; i++) {
                            for (int j = -UnitSize; j <= UnitSize; j++) {
                                chunk->SetBlock(g_blocks.RED_STAINED_GLASS, block_pos + BlockPos{i, 0, j});
                            }
                        }
                    }
                    else {
