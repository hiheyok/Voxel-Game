...
void OverworldGenerator::replaceBiomeBlocks(int x, int z, TallChunk* chunk,
    ...
    depthBuffer =
        surfaceNoise.getRegion(depthBuffer, (double)(x * 16), (double)(z * 16),
                               16, 16, 0.0625, 0.0625, 1.0);
...
void OverworldGenerator::generateHeightmap(int x, int y, int z) {
    ...
    mainNoiseRegion = mainPerlinNoise.generateNoiseOctaves(
        ...
        (double)(f / settings_.mainNoiseScaleX),
        (double)(f1 / settings_.mainNoiseScaleY),
        (double)(f / settings_.mainNoiseScaleZ));
    minLimitRegion = minLimitPerlinNoise.generateNoiseOctaves(
        minLimitRegion, x, y, z, 5, 33, 5, (double)f, (double)f1, (double)f);
    maxLimitRegion = maxLimitPerlinNoise.generateNoiseOctaves(
        maxLimitRegion, x, y, z, 5, 33, 5, (double)f, (double)f1, (double)f);
    int i = 0;
    int j = 0;
        ...
        for (int l = 0; l < 5; ++l) {
            ...
            for (int j1 = -2; j1 <= 2; ++j1) {
                for (int k1 = -2; k1 <= 2; ++k1) {
                    Biome* biome1 =
                        biomesForGeneration[k + j1 + 2 + (l + k1 + 2) * 10];
                    float f5 =
                        settings_.biomeDepthOffset +
                        biome1->getBaseHeight() * settings_.biomeDepthWeight;
                    float f6 = settings_.biomeScaleOffset +
                               biome1->getHeightVariation() *
                                   settings_.biomeScaleWeight;

                    float f7 =
                        biomeWeights[j1 + 2 + (k1 + 2) * 5] / (f5 + 2.0F);
            ...
            for (int l1 = 0; l1 < 33; ++l1) {
                double d1 = ((double)l1 - d0) * (double)settings_.stretchY *
                            128.0 / 256.0 / d9;
                ...
                double d2 =
                    minLimitRegion[i] / (double)settings_.lowerLimitScale;
                double d3 =
                    maxLimitRegion[i] / (double)settings_.upperLimitScale;
                    ...
                    for (int k2 = 0; k2 < 4; ++k2) {
                        ...
                        for (int l2 = 0; l2 < 4; ++l2) {
                            if ((lvt_45_1_ += d16) > 0.0) {
                                ...
                                chunk->SetBlockUnsafe(i * 4 + k2, genHeight,
                                                      l * 4 + l2,
                                                      g_blocks.STONE);
...
double OverworldGenerator::clampedLerp(double lowerBnd, double upperBnd,
    ...
    if (slide < 0.0) {
        ...
        return slide > 1.0 ? upperBnd
                           : lowerBnd + (upperBnd - lowerBnd) * slide;
    }
}