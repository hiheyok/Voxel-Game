...
void MountainGenerator::Generate(const ChunkPos& pos,
    ...
    for (int x = 0; x < kChunkDim; x++) {
        for (int z = 0; z < kChunkDim; z++) {
            float continental = ContinentialNoise(GetNoise2D(
                glm::ivec2(pos.x, pos.z), glm::ivec2(x, z), 3, 0.3f));
            float erosion = ErosionNoise(GetNoise2D(
                                glm::ivec2(pos.x, pos.z),
                                glm::ivec2(x + 4345, z + 6443), 3, 1.f)) /
                            2.f;
            float pv = PeaksAndValley(GetNoise2D(
                           glm::ivec2(pos.x, pos.z),
            ...
            for (int y = 0; y < kChunkDim; y++) {
                ...
                if (n > 0.5f) {
                    if (n < 0.54f) {
                        ...
                        chunk->SetBlockUnsafe(g_blocks.STONE,
                                              BlockPos{x, y, z});
                    }
                }
            }
            ...
            for (block_pos.y = 0; block_pos.y < kChunkDim; block_pos.y++) {
                if (block_pos.y + pos.y * kChunkDim < 34) {
                    if ((chunk->GetBlockUnsafe(block_pos) == g_blocks.AIR)) {
                        chunk->SetBlockUnsafe(g_blocks.BLUE_CONCRETE,
                                              block_pos);
                    }
...
void MountainGenerator::GenerateDecor(const ChunkPos& pos, Chunk* chunk) {
    ...
    for (int x = 0; x < kChunkDim; x++) {
        for (int z = 0; z < kChunkDim; z++) {
            ...
            float TREE_MAP =
                (float)((double)(noise_->GetNoise((float)gx * 100.f,
                                                  (float)gz * 100.f,
                                                  3453454.f) +
                                 1.f) /
                        2.f);
                ...
                if (chunk->GetBlock(BlockPos{x, y - 1, z}) == g_blocks.GRASS) {
                    if (TREE_MAP <= 0.04) {
                        ...
                        for (int ty = 0; ty < tree_height + 2; ty++) {
                            chunk->SetBlock(g_blocks.OAK_LOG,
                                            BlockPos{x, y + ty, z});
                        }
                    }
                }
...
size_t MountainGenerator::GetIndex(std::vector<glm::vec2>& vec,
                                   float bottomBound) {
