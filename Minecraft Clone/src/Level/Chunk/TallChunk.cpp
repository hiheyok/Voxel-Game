...
TallChunk::TallChunk(GameContext& game_context) : game_context_{game_context} {
  chunk_sub_column_.resize(16);
  for (int i = 0; i < 16; i++)
    chunk_sub_column_[i] = std::make_unique<Chunk>(game_context);
}
...
void TallChunk::SetBlockUnsafe(int x, int y, int z, BlockID block) {
  ...
  chunk_sub_column_[ChunkIndex]->SetBlockUnsafe(block,
                                                BlockPos{x, y & 0b1111, z});
}

BlockID TallChunk::GetBlockUnsafe(int x, int y, int z) {
  int ChunkIndex = y / 16;
  return chunk_sub_column_[ChunkIndex]->GetBlockUnsafe(
      BlockPos{x, y & 0b1111, z});
}
