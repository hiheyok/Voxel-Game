...
Item ItemList::RegisterBlockItem(BlockID block) {
	...
	item.properties_.name_ = Blocks.GetBlockType(block)->block_name_;
	...
	Logger.LogInfo("Item", "Registered Block Item: " + std::to_string(item_count_) + " | Name: " + item.properties_.name_);
...
void ItemList::RegisterAll() {
	for (BlockID b = 0; b < Blocks.block_type_data_.size(); b++) {
