...
class RenderDrawElementsInstanced : public RenderObject {
 public:
  // Triangles are the default draw type
  RenderDrawElementsInstanced(GameContext&, uint32_t draw_type = GL_TRIANGLES);
  ~RenderDrawElementsInstanced();
  ...
  void SetInstancesCount(size_t count) noexcept;
...
template <typename VertexType, typename ElementType>
void RenderDrawElementsInstanced::SetData(const std::vector<VertexType>& vertices,
                                 const std::vector<ElementType>& elements) {
