...
RawTextureData::~RawTextureData() { Erase(); }

RawTextureData::RawTextureData(const std::string& path) { Load(path); }
...
void RawTextureData::AnalyzeTransparency() {
  ...
  // terminates loop early if it found both fully and partially transparent
  // pixels
  for (int i = 0; i < width_ * height_ && (!partially_transparent_pixels_ ||
                                           !fully_transparent_pixels_);
       ++i) {
...
uint8_t RawTextureData::operator[](int index) const { return data_[index]; }
...
GLuint Texture::Get() const { return texture_id_; }

size_t Texture::GetHeight() const { return height_; }

size_t Texture::GetWidth() const { return width_; }

int Texture::GetFormat() const { return format_; }
