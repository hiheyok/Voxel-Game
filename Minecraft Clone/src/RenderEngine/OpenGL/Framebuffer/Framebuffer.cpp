...
void TexturedFrameBuffer::GenBuffer(GLint x, GLint y, float muti,
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, format, (int)(x * muti), (int)(y * muti), 0,
    ...
    glRenderbufferStorage(
        GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, (int)(x * muti),
        (int)(y * muti));  // use a single renderbuffer object for both a depth
                           // AND stencil buffer.
...
void TexturedFrameBuffer::BindFBO() {
    ...
    if (sx != text_size_x_ || sy != text_size_y_) {
        glBindTexture(GL_TEXTURE_2D, texture_);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, (int)(sx * res_multiplier_),
                     (int)(sy * res_multiplier_), 0, GL_RGB, GL_UNSIGNED_BYTE,
                     0);
        glBindRenderbuffer(GL_RENDERBUFFER, rbo_);
        glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8,
                              (int)(sx * res_multiplier_),
                              (int)(sy * res_multiplier_));
        text_size_y_ = sy;
        text_size_x_ = sx;
    }
}
