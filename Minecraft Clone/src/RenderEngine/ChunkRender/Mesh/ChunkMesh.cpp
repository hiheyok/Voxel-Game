            ...
            for (pos[axisU] = 0; pos[axisU] < kChunkDim; ++pos[axisU]) {
                for (pos[axisV] = 0; pos[axisV] < kChunkDim; ++pos[axisV]) {
                    if (usedBlock[(pos[axisU] << 4) + pos[axisV]] != 0x00) {
                        pos[axisV] +=
                            usedBlock[(pos[axisU] << 4) + pos[axisV]] - 1;
                        continue;
                    }
                    ...
                    const BlockModel& currModel =
                        g_blocks.GetBlockModel(currBlock);
                    const BlockModel& backModel =
                        g_blocks.GetBlockModel(backBlock);

                    bool blankCurrModel =
                        !currModel.is_initialized_ || pos[axis] == kChunkDim;
                    bool blankBackModel =
                        !backModel.is_initialized_ || pos[axis] == 0;
                    ...
                    if (!blankCurrModel) {
                        for (int i = 0; i < currModel.elements_.size(); ++i) {
                            ...
                            if (element.faces_[axis * 2 + 1]
                                    .reference_texture_.length() == 0)
                                continue;

                            if (element.faces_[axis * 2 + 1].cull_face_ != -1) {
                                if (!IsFaceVisible(element, pos,
                                                   element.faces_[axis * 2 + 1]
                                                       .cull_face_))
                                    continue;
                            }
                    ...
                    for (q_pos[axisV] = pos[axisV] + 1;
                         q_pos[axisV] < kChunkDim; ++q_pos[axisV]) {
                    ...
                    for (q_pos[axisU] = pos[axisU] + 1;
                         q_pos[axisU] < kChunkDim; ++q_pos[axisU]) {
                        bool isValid = true;

                        for (q_pos[axisV] = pos[axisV];
                             q_pos[axisV] < pos[axisV] + vLength;
                             ++q_pos[axisV]) {
                            const BlockID& currBlock2 = GetCachedBlockID(q_pos);
                            --q_pos[axis];
                            const BlockID& backBlock2 = GetCachedBlockID(q_pos);
                            ...
                            if (currBlock2 != currBlock ||
                                backBlock2 != backBlock) {
                                isValid = false;
                                break;
                            }
                        }
                        ...
                        memset(usedBlock.data() +
                                   (static_cast<long long>(q_pos[axisU]) << 4) +
                                   pos[axisV],
                               vLength, vLength);
                    ...
                    if (!blankCurrModel) {
                        for (int i = 0; i < currModel.elements_.size(); i++) {
                            ...
                            for (q_pos[axisU] = pos[axisU];
                                 q_pos[axisU] < pos[axisU] + uLength;
                                 ++q_pos[axisU]) {
                                for (q_pos[axisV] = pos[axisV];
                                     q_pos[axisV] < pos[axisV] + vLength;
                                     ++q_pos[axisV]) {
                                    AddFaceToMesh(element.faces_[axis * 2 + 1],
                                                  axis * 2 + 1, element.from_,
                                                  element.to_,
                                                  currModel.ambient_occlusion_,
                                                  q_pos);
                                }
                            }
                        }
...
void Mesh::ChunkMeshData::AddFaceToMesh(const BlockFace& face, uint8_t axis_,
                                        ...
                                        bool allowAO, const BlockPos& pos) {
    ...
    glm::ivec3 P0{x + from_[0] + kPositionOffset,
                  y + from_[1] + kPositionOffset,
    ...
    uint32_t tint = (static_cast<uint32_t>(!!face.tint_index_))
                    << kTintBitOffset;
...
glm::u8vec4 Mesh::ChunkMeshData::GetAO(uint8_t direction,
...
bool Mesh::ChunkMeshData::IsFaceVisible(const Cuboid& cube,
                                        const BlockPos& block_pos,
                                        uint8_t side) {
    ...
    for (int i = 0; i < model.elements_.size(); ++i) {
        ...
        if (element.faces_[oppositeSide].cull_face_ !=
                oppositeSide ||  // TODO: Replace and with OR  later after
...
bool Mesh::ChunkMeshData::CompareBlockSide(const BlockPos& pos, uint8_t side,
                                           BlockID b) {
