...
struct CommandPtr {
    ...
    CommandPtr(DrawCommandIndirect cmd_, ChunkPos pos)
        : cmd_(cmd_), position_(pos) {
        for (const auto& side : Directions<ChunkPos>()) {
            neighbor_[side] = nullptr;
        }
    }
    ...
    void SetNeighbor(CommandPtr* node, int side, int depth = 0) {
        ...
        if ((p4 == nullptr) && (p3 != nullptr)) {
            ...
            if (p1_distance_p3 <=
                p1_distance_p2) {  // this implies that p3 is the closest is is
                                   // kept the same
                ...
                p1->neighbor_[side] =
                    p2;  // no data lost occur here since if p4 is null, it
                         // implies p2 neighbor is null
                p2->neighbor_[!side] = p1;
...
class ChunkDrawCommandBuffer {
   public:
    ...
    size_t GetDrawCommandsSorted(std::vector<DrawCommandIndirect>& cmds,
        ...
        for (long long i = static_cast<long long>(commands_.size()) - 1; i >= 0;
             i--) {
    ...
    bool VerifyTree(CMDGraph::CommandPtr* node) {  // debug
        ...
        for (const auto& side : Directions<ChunkPos>()) {
            ...
            if (node->neighbor_[side]->neighbor_[!side] != node) {
                ...
                if (node->neighbor_[side]->visit_id_ !=
                    current_node_visit_id_) {
