...
ChunkMemoryPoolOffset::ChunkMemoryPoolOffset(size_t offset, size_t size) : mem_offset_(offset), mem_size_(size) {};
ChunkMemoryPoolOffset::ChunkMemoryPoolOffset(size_t offset, size_t size, ChunkPos position) : mem_offset_(offset), mem_size_(size), position_(position) {};
...
std::map<size_t, MemoryBlock>::const_reverse_iterator BlockManagement::rbegin() const {
    return mem_blocks_.rbegin();
}

std::map<size_t, MemoryBlock>::const_reverse_iterator BlockManagement::rend() const {
    return mem_blocks_.rend();
}

std::map<size_t, MemoryBlock>::const_iterator BlockManagement::lower_bound(size_t k) const { //Get latest element less than k
    auto it = mem_blocks_.lower_bound(k);
    it--;
    return it;
}

std::map<size_t, MemoryBlock>::const_iterator BlockManagement::upper_bound(size_t k) const { //Get latest element greater than k
    return mem_blocks_.upper_bound(k);
}

size_t BlockManagement::size() const {
    return mem_blocks_.size();
}

size_t BlockManagement::GetFragmentCount() const {
    return mem_blocks_.size();
}
...
void MemoryPoolManager::AllocateSpace(size_t memOffset, size_t memSize) { //Assumes input are valid
...
ChunkMemoryPoolOffset ChunkGPUMemoryPool::AddChunk(const std::vector<uint32_t>& vertices, ChunkPos pos) { //assumes vertices.size() != 0
