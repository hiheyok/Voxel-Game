...
void BakedChunkData::add(int x, int y, int z, BlockID block, unsigned int L0_, unsigned int L1_, unsigned int L2_, unsigned int L3_, char side) {
	if (x >= 16 || y >= 16 || z >= 16 || x < 0 || y < 0 || z < 0) {

	}
	else {
		data[(x * 256) + (z * 16) + y].Vdata[side] = 0 | block | (L0_ << 16) | (L1_ << 20) | (L2_ << 24) | (L3_ << 28);
		data[(x * 256) + (z * 16) + y].data = block;
	}

}
void BakedChunkData::addUninitBlock(int x, int y, int z, BlockID block) {
	if (x >= 16 || y >= 16 || z >= 16 || x < 0 || y < 0 || z < 0) {
...
unsigned int BakedChunkData::extractVData(char type, unsigned int data) {
	if (type == BMESH_ID)
		return (((1u << 16) - 1u) & (data >> 0));
	if (type == BMESH_L0)
		return (((1u << 4) - 1u) & (data >> 16));
	if (type == BMESH_L1)
		return (((1u << 4) - 1u) & (data >> 20));
	if (type == BMESH_L2)
		return (((1u << 4) - 1u) & (data >> 24));
	if (type == BMESH_L3)
		return (((1u << 4) - 1u) & (data >> 28));
	return 0;
}

bool BakedChunkData::isTransparent(int x, int y, int z) {
	return BlockList[extract(x, y, z).data]->block->transparency;
}
