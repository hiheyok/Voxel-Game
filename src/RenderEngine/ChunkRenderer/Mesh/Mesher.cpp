...
void ChunkMesh::SmartGreedyMeshing() {
	...
	for (int x = 0; x < 16; x++) {
		for (int z = 0; z < 16; z++) {
			for (int y = 0; y < 16; y++) {
				Block block = chunk->Blocks.GetBlock(x, y, z);

				if (block.Get()->isSolid && (!block.Get()->transparency)) {

					if (chunk->Blocks.GetBlock(x - 1, y, z).Get()->transparency)

					if (chunk->Blocks.GetBlock(x - 1, y, z).Get()->transparency)
						SaddSidenx(x, y, z, block);
					if (chunk->Blocks.GetBlock(x + 1, y, z).Get()->transparency)
						SaddSidepx(x, y, z, block);
					if (chunk->Blocks.GetBlock(x, y - 1, z).Get()->transparency)
						SaddSideny(x, y, z, block);
					if (chunk->Blocks.GetBlock(x, y + 1, z).Get()->transparency)
						SaddSidepy(x, y, z, block);
					if (chunk->Blocks.GetBlock(x, y, z - 1).Get()->transparency)
				...
				if (block.Get()->isSolid && (block.Get()->transparency)) {
					if (chunk->Blocks.GetBlock(x - 1, y, z).ID == AIR.ID)
						SaddSidenx(x, y, z, block);
					if (chunk->Blocks.GetBlock(x + 1, y, z).ID == AIR.ID)
						SaddSidepx(x, y, z, block);
					if (chunk->Blocks.GetBlock(x, y - 1, z).ID == AIR.ID)
						SaddSideny(x, y, z, block);
					if (chunk->Blocks.GetBlock(x, y + 1, z).ID == AIR.ID)
						SaddSidepy(x, y, z, block);
					if (chunk->Blocks.GetBlock(x, y, z - 1).ID == AIR.ID)
						SaddSidenz(x, y, z, block);
					if (chunk->Blocks.GetBlock(x, y, z + 1).ID == AIR.ID)
						SaddSidepz(x, y, z, block);
	...
	for (int x = 0; x < 16; x++) {
		for (int y = 0; y < 16; y++) {
			for (int z = 0; z < 16; z++) {
				...
				//PY
				if ((SMesh->extractVData(BMESH_ID, VDATA[PY]) != AIR.ID)) {
					tpy_ = BLOCK.get_PY();
					x0 = x;
					y0 = y;
					z0 = z;
				...
				//PX
				if ((SMesh->extractVData(BMESH_ID, VDATA[PX]) != AIR.ID)) {
					tpx_ = BLOCK.get_PX();
					x0 = x;
					y0 = y;
					z0 = z;
				...
				//PZ
				if ((SMesh->extractVData(BMESH_ID, VDATA[PZ]) != AIR.ID)) {
					tpz_ = BLOCK.get_PZ();
					x0 = x;
					y0 = y;
					z0 = z;
				...
				//NEGATIVES
				//NY
				if ((SMesh->extractVData(BMESH_ID, VDATA[NY]) != AIR.ID)) {
					tny_ = BLOCK.get_NY();
					x0 = x;
					y0 = y;
					z0 = z;
				...
				//NX
				if ((SMesh->extractVData(BMESH_ID, VDATA[NX]) != AIR.ID)) {
					tnx_ = BLOCK.get_NX();
					x0 = x;
					y0 = y;
					z0 = z;
				...
				//PZ
				if ((SMesh->extractVData(BMESH_ID, VDATA[NZ]) != AIR.ID)) {
					tnz_ = BLOCK.get_NZ();
					x0 = x;
					y0 = y;
					z0 = z;
...
void ChunkMesh::SaddSideny(int x, int y, int z, Block block) {
	int a = lightMvalue_;
	int b = lightMvalue_;
	int a1 = lightMvalue_;
	int b1 = lightMvalue_;
	if (!chunk->Blocks.GetBlock(x - 1, y - 1, z).Get()->transparency) {
		a = lightCvalue_;
		b = lightCvalue_;
	}
	if (!chunk->Blocks.GetBlock(x + 1, y - 1, z).Get()->transparency) {
	...
	if (!chunk->Blocks.GetBlock(x, y - 1, z + 1).Get()->transparency) {
		a1 = lightCvalue_;
		b = lightCvalue_;
	}
	if (!chunk->Blocks.GetBlock(x, y - 1, z - 1).Get()->transparency) {
	...
	if (!chunk->Blocks.GetBlock(x + 1, y - 1, z + 1).Get()->transparency)
		a1 = lightCvalue_;
	if (!chunk->Blocks.GetBlock(x - 1, y - 1, z - 1).Get()->transparency)
		a = lightCvalue_;
	if (!chunk->Blocks.GetBlock(x + 1, y - 1, z - 1).Get()->transparency)
...
void ChunkMesh::SaddSidepy(int x, int y, int z, Block block) {
	int a = lightMvalue_;
	int b = lightMvalue_;
	int a1 = lightMvalue_;
	int b1 = lightMvalue_;
...
void ChunkMesh::SaddSidenx(int x, int y, int z, Block block) {
	int br = lightMvalue_;
	int bl = lightMvalue_;
	int tl = lightMvalue_;
	int tr = lightMvalue_;
	...
	if (!chunk->Blocks.GetBlock(x - 1, y, z + 1).Get()->transparency) {
		tr = lightCvalue_;
		br = lightCvalue_;
	}
	if (!chunk->Blocks.GetBlock(x - 1, y, z - 1).Get()->transparency) {
...
void ChunkMesh::SaddSidepx(int x, int y, int z, Block block) {
	int br = lightMvalue_;
	int bl = lightMvalue_;
	int tl = lightMvalue_;
	int tr = lightMvalue_;
...
void ChunkMesh::SaddSidenz(int x, int y, int z, Block block) {
	int br = lightMvalue_;
	int bl = lightMvalue_;
	int tl = lightMvalue_;
	int tr = lightMvalue_;
...
void ChunkMesh::SaddSidepz(int x, int y, int z, Block block) {
	int br = lightMvalue_;
	int bl = lightMvalue_;
	int tl = lightMvalue_;
	int tr = lightMvalue_;
